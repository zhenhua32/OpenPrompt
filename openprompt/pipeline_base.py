...
class PromptDataLoader(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert hasattr(
            self.template, "wrap_one_example"
        ), "Your prompt has no function variable \
    ...
    def wrap(self):
        ...
        if isinstance(self.raw_dataset, Dataset) or isinstance(self.raw_dataset, List):
            ...
            for idx, example in enumerate(self.raw_dataset):
                if self.verbalizer is not None and hasattr(
                    self.verbalizer, "wrap_one_example"
                ):  # some verbalizer may also process the example.
...
class PromptForGeneration(nn.Module, GenerationMixin):
    ...
    def generate(self, batch: Union[Dict, InputFeatures], verbose: Optional[bool] = False, **generation_kwargs):
        ...
        if self.config.is_encoder_decoder:
            ...
            for instance_id in range(batch_size):
                ...
                instance = {
                    key: batch[key][instance_id : instance_id + 1][:, : input_real_lens[instance_id]]
                    for key in batch
                ...
                self.generate_ith_token = 0
                self.in_generation_function = True
                output_sequence = super().generate(
                    **instance,
                    **input_generation_kwargs,
                    pad_token_id=self.tokenizer.pad_token_id,
                    eos_token_id=self.tokenizer.eos_token_id,
            ...
            generated_sentences = self.post_processing(
                output_sequences=output_sequences, input_lengths=input_real_lens.cpu().tolist()
            )
        if verbose:
    ...
    def prepare_inputs_for_generation(self, input_ids: Optional[torch.Tensor] = None, **model_kwargs):
        ...
        self.last_model_inputs = (
            model_inputs  # to update the model_kwargs in _update_model_kwargs_for_generation, in-place operation.
        )
        return model_inputs
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            encoder_kwargs = {
                ...
                if not (
                    ...
                    or argument.startswith("use_cache")
                )
            }
